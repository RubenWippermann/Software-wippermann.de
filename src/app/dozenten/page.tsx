...
export default function Dozenten(){
  ...
  return <main className={s.page} style={{"--accent":"#f20b7a","--dark":"#9c0750","--soft":"#fff0f7"} as CSSProperties}>
    ...
    <div className={s.panel}>
      <aside className={s.panelSide}><div className={s.panelProfile}><div className={s.avatar}>LB</div><div><strong>Lukas Demo</strong><small>Lehrkraft · aktiv</small></div></div><div className={s.sideNav}>{bereiche.map(x=><button type="button" key={x} className={bereich===x?s.active:""} onClick={()=>setBereich(x)}>{x}</button>)}</div></aside>
